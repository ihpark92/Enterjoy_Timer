...
#### v0.942 (2025-01-18)
- **성좌 타이머 알림 조건 개선 (중요 버그 수정)**
  - enterjoy 탭이 없을 때 알림이 표시되지 않도록 수정
  - 브라우저 종료 또는 enterjoy 탭을 모두 닫으면 알람 자동 중지
  - PC 재시작 후 브라우저 미실행 상태에서 알림이 뜨는 문제 해결
- **탭 추적 시스템 개선**
  - `chrome.tabs.onRemoved` 리스너 추가: 탭이 닫힐 때 즉시 감지하여 목록에서 제거
  - `chrome.tabs.onUpdated` 리스너 개선: enterjoy에서 다른 사이트로 이동 시 탭 목록에서 제거
  - 모든 enterjoy 탭이 닫히면 알람 자동 중지 및 리소스 절약
- **유효성 검증 강화**
  - `hasValidEnterjoyTabs()` 함수 추가: 타이머 만료 시 실제로 유효한 enterjoy 탭만 확인
  - 닫힌 탭 또는 URL이 변경된 탭은 자동으로 제거
  - 유효한 탭이 없으면 알람 자동 중지
- **알림 표시 로직 최적화**
  - 알림 조건: enterjoy 탭이 존재하고 비활성화되어 있을 때만 표시
  - 리소스 효율성: 탭이 없으면 불필요한 알람 실행 방지
