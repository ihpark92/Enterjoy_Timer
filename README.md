...
- **성좌 출현 타이머**: 30분 간격 고정으로 포인트 수집 시간 알림
  - 다른 탭 활성 시: Badge 깜빡임, 탭 제목 깜빡임, 파비콘 변경으로 알림
  - enterjoy 탭 활성 시: 기존 배너 표시
...
- **성좌 타이머 알림 강화**
  ...
  - 다른 탭 활성 시 다층 시각적 알림:
    - 확장 아이콘 Badge 깜빡임 (🎁, 1초 간격)
    - enterjoy 탭 제목 깜빡임 ("🎁🎁🎁 성좌 출현! 클릭하세요! 🎁🎁🎁", 1.5초 간격)
    - enterjoy 탭 파비콘 변경 (빨간 배경에 🎁)
  - enterjoy 탭 활성 시: 기존 배너 표시 유지
  - 60초 후 자동 중지 또는 탭 클릭 시 즉시 중지
  - 비활성 탭에서도 정확한 타이머 동작 보장
- **권한 추가**
  - `alarms` 권한 추가 (사용자 승인 불필요)
  - 정확한 30분 간격 타이머 보장
  - 배터리 효율적인 타이머 관리
